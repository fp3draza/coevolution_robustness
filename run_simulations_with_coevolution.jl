...
@everywhere function initialise_parameters_for_simulation(num_resources, num_consumers, connectance,  m_mean, alpha_val, replicate, distribution)
        ...
        init = draw_initial_triat_values(num_resources, num_consumers, replicate, distribution)
        ...
        # draw m values
        m_distribution = Normal(m_mean, 0.01)

        # seed
        Random.seed!(replicate)
        m = rand(m_distribution, num_resources + num_consumers)
        ...
        return init, theta, alpha, phi, m, t_max, f, sigma
...
@everywhere function store_traits_and_network_to_file(traits, network, time, type_evo, type_connectance, m_val, alpha, replica, id, distribution)
    ...
    # store output
    writedlm(string("/Users/fernando/Documents/Projects/network_rewiring/output/",distribution,"/networks/",id,"_interaction_type_",type_evo,"_",type_connectance,"_connectance_m_",m_val,"_alpha_",alpha,"_replica_", replica,".csv"), network)
    writedlm(string("/Users/fernando/Documents/Projects/network_rewiring/output/",distribution,"/traits/",id,"_interaction_type_",type_evo,"_",type_connectance,"_connectance_m_",m_val,"_alpha_",alpha,"_replica_", replica,".csv"), traits)
...
@everywhere function update_dataframe(df, t, network_past, network_current, traits_past, traits_current, type_evo, alpha, type_connectance, num_resources, num_consumers, m_val, replica, id, theta, distribution)
    ...
    df[t, "modularity"] = modularity
    df[t, "nestedness"] = nestedness
    ...
    # write to file
    #CSV.write(string("/Users/fernando/Documents/Projects/network_rewiring/output/",distribution,"/summarised_output/",id,"_interaction_type_",type_evo,"_",type_connectance,"_connectance_m_",m_val,"_alpha_",alpha,"_replica_", replica,".csv"),  df[df.generation .== 200, :])

    # return
    return df[df.generation .== 200, :]
...
@everywhere function update_components_dataframe(df_components, t, f_new, f_old, traits, num_resources, num_consumers, type_evo, alpha, type_connectance, m_val, replica, id, theta, distribution)
    ...
    if t == 200
        CSV.write(string("/home/fernando/network_rewiring/output/",distribution,"/components/",id,"_interaction_type_",type_evo,"_",type_connectance,"_connectance_m_",m_val,"_alpha_",alpha,"_replica_", replica,".csv"),  df_components)
    end
...
@everywhere function initialise_dataframe_for_results(t_max, type_evo, type_connectance, distribution, fraction, strategy)
    ...
    df = DataFrame(generation=1:t_max,
                      ...
                      fraction=fraction,
                      strategy=strategy,
                      rewiring_events=Vector{Union{Missing, Int64}}(missing, t_max),
                      average_trait_change = Vector{Union{Missing, Float64}}(missing, t_max),
                      average_trait_matching = Vector{Union{Missing, Float64}}(missing, t_max),
                      average_theta_matching = Vector{Union{Missing, Float64}}(missing, t_max),
                      modularity = Vector{Union{Missing, Float64}}(missing, t_max),
                      nestedness = Vector{Union{Missing, Float64}}(missing, t_max),
...
@everywhere function nestedness_Fortuna(B, num_resources, num_consumers, convert)
    ...
    #nestedness_rows = nested_rows/(size(B,1)*(size(B,1)-1)/2)
    ...
    #nestedness_columns = nested_columns/(size(B,2)*(size(B,2)-1)/2)
...
@everywhere function run_simulation(num_resources, num_consumers, connectance, type_evo, type_connectance, m_val, alpha_val, replica, id, distribution, fraction, strategy)
    ...
    init, theta, alpha, phi, m, t_max, f, sigma = initialise_parameters_for_simulation(num_resources, num_consumers, connectance, m_val, alpha_val, replica, distribution)
    ...
    for t in 1:t_max
        ...
        # run the mutualistic coevolution model for one generation
        traits_after_coevolution = flip_interactions_and_simulate_coevolution(f, init, theta, alpha, m, phi, num_resources, num_consumers, sigma, type_evo, fraction, strategy)
...
@everywhere function run_routine_artifical(type_of_interaction, type_of_connectance, m, alpha, replicate, distribution, fraction, strategy)
    ...
    network_shapes = define_artifical_networks(type_of_interaction)
...
@everywhere function define_artifical_networks(type_of_interaction)
...
function run_simulations()
  ...
  for type_evo in ["mutualistic", "antagonistic"]
        ...
        for fraction in [0]
            ...
            for m in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]
                ...
                for alpha in [0.01000000, 0.01258925, 0.01584893, 0.01995262, 0.02511886, 0.03162278, 0.03981072, 0.05011872, 0.06309573, 0.07943282, 0.10000000, 0.12589254, 0.15848932, 0.19952623, 0.25118864, 0.31622777, 0.39810717, 0.50118723, 0.63095734, 0.79432823, 1.00000000]
                    ...
                    # store
                    CSV.write(string("/home/fernando/network_rewiring/output/",distribution,"/summarised_output/",type_evo,"/interaction_type_",type_evo,"_variable_connectance_m_",m,"_alpha_",alpha,".csv"), df_store)
